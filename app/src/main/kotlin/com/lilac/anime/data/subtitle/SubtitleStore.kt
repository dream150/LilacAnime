...
object SubtitleStore {
    ...
    /**
     * The filename generated by SubtitleDownloader is the authoritative episode
     * identity for Linkkf VTT files: sub_<animeId>_ep_<episodeKey>.vtt
     *
     * This is deliberately based on the generated filename rather than subtitle
     * contents or the remote VTT URL. A Linkkf VTT can have no useful episode
     * metadata, while its download request already knows the exact display key.
     */
    ...
    /**
     * Episode-key-aware matching. For the newly generated Linkkf VTT filename,
     * the exact key after `_ep_` wins. This keeps 4, 4a, 04, etc. from being
     * accidentally treated as the same subtitle when the source distinguishes them.
     */
    fun subtitleMatchesEpisode(path: String, episodeKey: String, episodeNumber: Int): Boolean {
        ...
        // Non-Linkkf/legacy subtitle files use the existing numeric/header checks.
